...
class CreateApp extends StatelessWidget {
  CreateApp(
      {super.key, required this.contextWallpaper, required this.intention});
  ...
  bool checkFields(String hour, String minute, String file, String cond) {
    // confirm all fields are filled
    if (isNumeric(hour) &&
        isNumeric(minute) &&
        minute.length == 2 // confirm 00 format
        &&
        cond != "" &&
        file != "") {
      return true;
    ...
    Widget buttonMenu = Container(
      ...
      child: Row(
        children: [
          ...
          OutlinedButton(
            onPressed: () {
              if (checkFields(hourController.text, minuteController.text,
                  chosenFile, chosenCond)) {
                confirmCreation(intention, contextWallpaper,
                    WallpaperObj()); // add fields to newWallpaperObj
              } else {
                showWarningWindow(context, 1);
              }
            },
